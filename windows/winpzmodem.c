...
void xyz_ReceiveInit(Terminal *term)
{
	const char *rzcmd = conf_get_filename(term->conf,CONF_rzcommand)->path;
	if (!strcmp(rzcmd, "")) {
		MessageBox(NULL, "rz 실행파일을 지정 하십시오.", "ZMODEM 에러", MB_OK|MB_ICONERROR);
		return;
	}

	if (xyz_SpawnProcess(term, rzcmd, conf_get_str(term->conf,CONF_rzoptions)) == 0) {
		term->xyz_transfering = 1;
	} else {
		MessageBox(NULL,"파일 수신을 시작할 수 없습니다!", "ZMODEM 에러", MB_OK|MB_ICONERROR);
	}
}
