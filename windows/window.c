...
{
    WNDCLASSEXW wndclass; //HACK: PuttyTray / Session Icon
    MSG msg;
    HRESULT hr;
...
#define f(j,c,w,u,i) \
    ...
    if (kbd_codepage == 949) {
	if (strncmp(font->name, "나눔", 4) == 0 ||
	    strcmp(font->name, "D2Coding") == 0 ||
	    strncmp(font->name, "Noto Sans CJK", 13) == 0 ||
	    strncmp(font->name, "굴림", 4) == 0 ||
	    strncmp(font->name, "궁서", 4) == 0 ||
	    strncmp(font->name, "돋움", 4) == 0 ||
	    strncmp(font->name, "맑은", 4) == 0 ||
	    strncmp(font->name, "문제부", 6) == 0 ||
	    strncmp(font->name, "바탕", 4) == 0 ||
	    strcmp(font->name, "새굴림") == 0 ||
	    strncmp(font->name, "함초", 4) == 0 ||
	    strncmp(font->name, "휴먼", 4) == 0) {
...
#define SHOW_TOASCII_RESULT
	...
	if (funky_type == FUNKY_HANTERM && code >= 11 && code <= 34) {
	    switch (wParam) {
